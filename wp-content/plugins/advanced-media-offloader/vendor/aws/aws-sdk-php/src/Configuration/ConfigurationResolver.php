...
namespace WPFitter\Aws\Configuration;
...
class ConfigurationResolver
{
    ...
    public static function ini($key, $expectedType, $profile = null, $filename = null, $options = [])
    {
        ...
        $data = @\WPFitter\Aws\parse_ini_file($filename, \true, \INI_SCANNER_NORMAL);
    ...
    {
        if ($type === 'bool' && !is_null($convertedValue = \WPFitter\Aws\boolean_value($value))) {
    ...
    private static function retrieveValueFromIniSubsection($data, $profile, $filename, $expectedType, $options)
    {
        ...
        $services_section = \WPFitter\Aws\parse_ini_section_with_subsections($filename, "services {$data[$profile]['services']}");
