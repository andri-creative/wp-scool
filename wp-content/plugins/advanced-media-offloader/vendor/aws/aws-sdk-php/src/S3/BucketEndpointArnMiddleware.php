...
use WPFitter\Aws\Arn\AccessPointArnInterface;
use WPFitter\Aws\Arn\ArnParser;
use WPFitter\Aws\Arn\ObjectLambdaAccessPointArn;
use WPFitter\Aws\Arn\Exception\InvalidArnException;
use WPFitter\Aws\Arn\AccessPointArn as BaseAccessPointArn;
use WPFitter\Aws\Arn\S3\OutpostsAccessPointArn;
use WPFitter\Aws\Arn\S3\MultiRegionAccessPointArn;
use WPFitter\Aws\Arn\S3\OutpostsArnInterface;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(BaseAccessPointArn $arn, RequestInterface $req)
    {
        ...
        $region = \WPFitter\Aws\strip_fips_pseudo_regions($region);
    ...
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!$clientPart->isRegionMatch($this->region, 's3')) {
                $clientPart = $this->partitionProvider->getPartition(\WPFitter\Aws\strip_fips_pseudo_regions($this->region), 's3');
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {
