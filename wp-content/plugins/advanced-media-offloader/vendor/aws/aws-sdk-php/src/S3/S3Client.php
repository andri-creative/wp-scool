...
use WPFitter\Aws\ClientResolver;
use WPFitter\Aws\Command;
...
use WPFitter\Aws\HandlerList;
use WPFitter\Aws\Identity\S3\S3ExpressIdentityProvider;
use WPFitter\Aws\InputValidationMiddleware;
use WPFitter\Aws\Middleware;
use WPFitter\Aws\ResultInterface;
use WPFitter\Aws\Retry\QuotaManager;
use WPFitter\Aws\RetryMiddleware;
use WPFitter\Aws\RetryMiddlewareV2;
use WPFitter\Aws\S3\Parser\GetBucketLocationResultMutator;
use WPFitter\Aws\S3\Parser\S3Parser;
use WPFitter\Aws\S3\Parser\ValidateResponseChecksumResultMutator;
use WPFitter\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use WPFitter\Aws\S3\UseArnRegion\Configuration;
use WPFitter\Aws\S3\UseArnRegion\ConfigurationInterface;
use WPFitter\Aws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use WPFitter\GuzzleHttp\Exception\RequestException;
...
class S3Client extends AwsClient implements S3ClientInterface
{
    ...
    public function createPresignedRequest(CommandInterface $command, $expires, array $options = [])
    {
        ...
        $request = \WPFitter\Aws\serialize($command);
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \WPFitter\Aws\serialize($command)->getUri();
    }
    /**
    ...
    {
        if ($value) {
            $config = \WPFitter\Aws\Retry\ConfigurationProvider::unwrap($value);
