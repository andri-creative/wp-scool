...
use WPFitter\Aws\Api\Parser\PayloadParserTrait;
...
use WPFitter\Aws\S3\Exception\PermanentRedirectException;
...
trait S3ClientTrait
{
    ...
    public function deleteMatchingObjectsAsync($bucket, $prefix = '', $regex = '', array $options = [])
    {
        ...
        if ($regex) {
            $iter = \WPFitter\Aws\filter($iter, function ($c) use ($regex) {
