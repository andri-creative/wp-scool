...
use WPFitter\GuzzleHttp\Client;
use WPFitter\Psr\Http\Message\RequestInterface;
use WPFitter\GuzzleHttp\ClientInterface;
...
function default_http_handler()
{
    ...
    if ($version === 6 || $version === 7) {
        return new \WPFitter\Aws\Handler\GuzzleV6\GuzzleHandler();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return new \WPFitter\Aws\Handler\GuzzleV5\GuzzleHandler();
    }
...
function default_user_agent()
{
    ...
    // If Guzzle 6 or 7 installed
    if ($version === 6 || $version === 7) {
        return \WPFitter\GuzzleHttp\default_user_agent();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return \WPFitter\GuzzleHttp\Client::getDefaultUserAgent();
    }
    throw new \RuntimeException('Unknown Guzzle version: ' . $version);
}
/**
...
function guzzle_major_version()
{
    ...
    if (defined('WPFitter\GuzzleHttp\ClientInterface::VERSION')) {
    ...
    } elseif (defined('WPFitter\GuzzleHttp\ClientInterface::MAJOR_VERSION')) {
